...
1. Run the validation script (from UPR-OS repo):
```bash
/Users/skc/Projects/UPR/upr-os/scripts/validate-deployment.sh staging
```
...
```bash
# Check SaaS staging
curl -s https://premiumradar-saas-staging-191599223867.us-central1.run.app/api/health | jq .

# Check OS service
curl -s https://upr-os-service-191599223867.us-central1.run.app/health | jq .

# Test discovery endpoint
curl -s -X POST https://premiumradar-saas-staging-191599223867.us-central1.run.app/api/os/discovery \
  -H "Content-Type: application/json" \
  -d '{"tenant_id":"test","vertical_id":"banking","region_code":"UAE"}' | jq .
```
